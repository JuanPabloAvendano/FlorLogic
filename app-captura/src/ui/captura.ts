...
import type { Captura, LineaCaptura } from '../modelo'
import type { Hallazgo } from '../reglas'
import { MotorReglas } from '../reglas'
import { aviso, boton, campo, entradaNumero, etiqueta, h, selector, tarjeta, titulo, vaciar, vacio } from '../vista'
import { escanearCodigo, hayEscaner } from '../escaner'
import type { Ctx, Variante } from './comun'
import { hoy, NOMBRE_VARIANTE, numero } from './comun'

class Sesion {
  constructor(
    private readonly ctx: Ctx,
    readonly captura: Captura,
    public lineas: LineaCaptura[],
    private readonly otrasHoy: number,
  ) {}
  ...
  async agregar(variedadId: string): Promise<LineaCaptura> {
  ...
  async actualizar(linea: LineaCaptura, cambios: Partial<LineaCaptura>): Promise<LineaCaptura> {
  ...
  async quitar(linea: LineaCaptura): Promise<void> {
  ...
  sugerencia(linea: LineaCaptura): number | null {
...
export async function pantallaCaptura(ctx: Ctx, camaId: string, variante: Variante): Promise<HTMLElement> {
  ...
  raiz.append(titulo(
    `Capturar cama ${cama.codigo}`,
    `Bloque ${bloque?.codigo ?? '—'} · ${captura.fecha} · ${NOMBRE_VARIANTE[variante]}`,
  ))
...
function rejilla(ctx: Ctx, sesion: Sesion, pintar: () => void, cerrar: () => void): HTMLElement {
  const caja = h('section', { clase: 'captura' })
  caja.append(
    h('div', { clase: 'captura__cabeza' },
      etiqueta('aviso', 'borrador'),
      h('span', { clase: 'captura__guardado', texto: 'Se guarda solo, dato por dato' }),
    ),
  )
  ...
  rejillaEl.append(
    h('div', { clase: 'rejilla__cabecera' },
      h('span', { texto: 'VARIEDAD' }),
      h('span', { texto: '# LÍNEAS' }),
      h('span', { texto: 'CANTIDAD' }),
      h('span', { texto: '' }),
    ),
  )
  for (const linea of sesion.lineas) rejillaEl.append(filaRejilla(ctx, sesion, linea, pintar, pie.refrescar))
    ...
    h('div', { clase: 'acciones' },
      ...
      sesion.lineas.length > 0
        ? boton('Deshacer última', async () => {
            ...
            if (ultima) await sesion.quitar(ultima)
            pintar()
          }, 'tenue')
        : null,
    ),
...
function filaRejilla(
  ...
  const repintarAvisos = () => {
    ...
    if (s !== null && linea.cantidad === null) {
      ...
      avisos.append(
        h('p', { clase: 'aviso aviso--neutro' },
          `Con ${ctx.catalogo.variedad(linea.variedadId)?.plantasPorLinea} por línea serían ${numero(s)}. `,
          boton('Usar', async () => {
  ...
  fila.append(
    selector(
      sesion.variedadesDelBloque().map((v) => ({ valor: v.id, texto: v.nombre })),
      linea.variedadId,
      async (v) => { linea = await sesion.actualizar(linea, { variedadId: v }); repintarAvisos(); refrescarPie() },
    ),
  )
...
function guiada(ctx: Ctx, sesion: Sesion, pintar: () => void, cerrar: () => void): HTMLElement {
  const caja = h('section', { clase: 'captura' })
  caja.append(
    h('div', { clase: 'captura__cabeza' },
      etiqueta('aviso', 'borrador'),
  ...
  if (hayEscaner()) {
    caja.append(
      h('div', { clase: 'acciones' },
        boton('Escanear la marca de la cama', async () => {
          const codigo = await escanearCodigo(caja)
          if (!codigo) return
          const destino = ctx.catalogo.semilla.camas.find((c) => c.codigo === codigo || c.id === codigo)
          if (destino) ctx.ir({ n: 'captura', camaId: destino.id, variante: 'guiada' })
          else caja.append(aviso('aviso', `No hay ninguna cama con el código ${codigo}.`, 'Buscala en la lista de bloques.'))
        }, 'tenue'),
      ),
    )
  }
    ...
    caja.append(
      h('div', { clase: 'seccion' },
        h('div', { clase: 'seccion__cabeza' },
          ...
          completa ? etiqueta('ok', 'lista') : etiqueta('aviso', 'incompleta'),
        ),
        h('dl', { clase: 'datos' },
          h('dt', { texto: '# líneas' }), h('dd', { texto: String(linea.lineas ?? '—') }),
          h('dt', { texto: 'Cantidad' }), h('dd', { texto: numero(linea.cantidad) }),
        ),
        h('div', { clase: 'acciones' },
          boton('Quitar', async () => { await sesion.quitar(linea); pintar() }, 'tenue'),
        ),
      ),
    )
  }
...
function formularioGuiado(ctx: Ctx, sesion: Sesion, pintar: () => void): HTMLElement {
  const variedades = sesion.variedadesDelBloque()
  ...
  const refrescarSugerencia = () => {
    ...
    if (n && v?.plantasPorLinea && cantidadTexto === '') {
      ...
      sugerenciaEl.append(
        h('p', { clase: 'aviso aviso--neutro' },
          `Con ${v.plantasPorLinea} por línea serían ${numero(s)}. `,
          boton('Usar', () => { cantidadTexto = String(s); entradaCantidad.value = cantidadTexto; refrescarSugerencia() }, 'tenue'),
          v.confianza === 'en-disputa' ? etiqueta('alerta', 'valor no confiable') : null,
        ),
      )
    }
  }
  ...
  const pintarBotonera = () => {
    ...
    for (const v of variedades.slice(0, 8)) {
      const b = boton(v.nombre, () => { variedadId = v.id; pintarBotonera(); refrescarSugerencia() },
        v.id === variedadId ? 'normal' : 'tenue')
      botonera.append(b)
    }
  }
  pintarBotonera()
  ...
  return h('div', { clase: 'formulario' },
    campo('Variedad', botonera),
    campo('# Líneas', entradaLineas),
    campo('Cantidad', entradaCantidad),
    sugerenciaEl,
    h('div', { clase: 'acciones' },
      boton('Agregar sección', async () => {
 ...
 */
function pieDeCaptura(sesion: Sesion, cerrar: () => void): { el: HTMLElement; refrescar: () => void } {
  const total = h('div', { clase: 'pie-captura__total' })
  const refrescar = () => {
    vaciar(total)
    total.append(
      h('span', { texto: `${sesion.lineas.length} ${sesion.lineas.length === 1 ? 'sección' : 'secciones'}` }),
      h('strong', { texto: `${numero(sesion.total())} plantas` }),
    )
  }
  refrescar()
 ...
 */
function confirmacion(ctx: Ctx, sesion: Sesion, volver: () => void): HTMLElement {
    ...
    resumen.append(
      tarjeta(null,
        h('div', { clase: 'tarjeta__fila' },
          h('span', { clase: 'tarjeta__codigo', texto: ctx.catalogo.variedad(l.variedadId)?.nombre ?? '—' }),
          h('strong', { texto: numero(l.cantidad) }),
        ),
        h('div', { clase: 'tarjeta__meta' }, h('span', { texto: `${l.lineas ?? '—'} líneas` })),
      ),
    )
  }
  caja.append(resumen)
  ...
  for (const hz of duras) caja.append(aviso('alerta', hz.mensaje, hz.queHacer))
  for (const hz of blandas) caja.append(aviso('aviso', hz.mensaje, hz.queHacer))
  ...
  const botonCerrar = boton('Guardar y cerrar', async () => {
    if (duras.length > 0 || !revisado) return
  ...
  const refrescarBoton = () => {
    ...
    if (puede) botonCerrar.removeAttribute('disabled')
    else botonCerrar.setAttribute('disabled', 'true')
  }
