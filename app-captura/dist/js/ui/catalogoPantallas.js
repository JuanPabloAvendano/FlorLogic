 ...
 */
import { boton, cifra, etiqueta, h, tabla, tarjeta, titulo, vacio } from '../vista.js';
import { numero } from './comun.js';
export function pantallaInicio(ctx) {
    ...
    for (const b of ctx.catalogo.bloques()) {
        ...
        lista.append(tarjeta(() => ctx.ir({ n: 'camas', bloqueId: b.id }), h('div', { clase: 'tarjeta__fila' }, h('span', { clase: 'tarjeta__codigo', texto: `Bloque ${b.codigo}` }), h('span', { clase: 'tarjeta__flecha', texto: '›', atributos: { 'aria-hidden': 'true' } })), h('div', { clase: 'tarjeta__meta' }, h('span', { texto: `${camas.length} camas` }), ...b.tiposFlor.filter(Boolean).map((t) => h('span', { texto: t })), divididas > 0 ? etiqueta('neutro', `${divididas} divididas`) : null)));
    }
    raiz.append(lista);
    return raiz;
}
export function pantallaCamas(ctx, bloqueId) {
    ...
    for (const c of camas) {
        ...
        lista.append(tarjeta(() => ctx.ir({ n: 'cama', camaId: c.id }), h('div', { clase: 'tarjeta__fila' }, h('span', { clase: 'tarjeta__codigo', texto: `Cama ${c.codigo}` }), h('span', { clase: 'tarjeta__flecha', texto: '›', atributos: { 'aria-hidden': 'true' } })), h('div', { clase: 'tarjeta__meta' }, h('span', {
            ...
            : null, ctx.catalogo.estaDividida(c.id) ? etiqueta('neutro', 'dividida') : null, conAviso ? etiqueta('aviso', 'revisar') : null)));
    }
    raiz.append(lista);
    return raiz;
}
export function pantallaCama(ctx, camaId) {
    ...
    raiz.append(h('div', { clase: 'acciones' }, boton('Capturar · rejilla', () => ctx.ir({ n: 'captura', camaId, variante: 'rejilla' })), boton('Capturar · guiada', () => ctx.ir({ n: 'captura', camaId, variante: 'guiada' })), boton('Papel contra app', () => ctx.ir({ n: 'papel', camaId }), 'tenue')));
    const porFecha = new Map();
    for (const s of ctx.catalogo.siembras(camaId))
        porFecha.set(s.fecha, [...(porFecha.get(s.fecha) ?? []), s]);
    for (const [fecha, items] of porFecha) {
        const seccion = h('section', { clase: 'jornada' });
        seccion.append(h('h2', { clase: 'jornada__fecha' }, fecha, items.length > 1 ? etiqueta('neutro', `${items.length} secciones`) : null));
        for (const s of items) {
            const v = ctx.catalogo.variedad(s.variedadId);
            const datos = h('dl', { clase: 'datos' });
            const par = (k, valor) => { datos.append(h('dt', { texto: k }), h('dd', { texto: valor })); };
            if (s.lineas !== null)
                par('# líneas', String(s.lineas));
            if (s.cantidad !== null)
                par('Cantidad', numero(s.cantidad));
            if (s.razonPlantasPorLinea !== null)
                par('Plantas por línea', String(s.razonPlantasPorLinea));
            if (s.lote)
                par('Lote', s.lote);
            if (s.calibre)
                par('Calibre', s.calibre);
            if (s.proveedor)
                par('Proveedor', s.proveedor);
            if (s.contenedor)
                par('Contenedor', s.contenedor);
            if (s.obse)
                par('OBSE', `${s.obse} (significado desconocido)`);
            if (s.observaciones)
                par('Observaciones', s.observaciones);
            const caja = h('div', { clase: 'seccion' }, h('div', { clase: 'seccion__cabeza' }, h('span', { clase: 'seccion__variedad', texto: v?.nombre ?? '—' }), h('span', { clase: 'seccion__hoja', texto: `${s.hojaOrigen} · fila ${s.filaOrigen}` })), datos);
 ...
 */
export async function pantallaPapel(ctx, camaId) {
    const cama = ctx.catalogo.cama(camaId);
    if (!cama)
        return vacio('Cama no encontrada', 'El catálogo local no la tiene.');
    const raiz = h('div', {});
    ...
    raiz.append(h('div', { clase: 'cifras' }, cifra(numero(totalPapel), 'plantas en el papel'), cifra(numero(totalApp), 'plantas en la app'), cifra(pct === null ? '—' : `${pct > 0 ? '+' : ''}${pct}%`, 'diferencia')));
    raiz.append(h('h2', { clase: 'seccion__titulo', texto: 'Formato en papel' }));
    raiz.append(tabla(['Fecha', 'Variedad', '# líneas', 'Cantidad'], papel.map((s) => [s.fecha, ctx.catalogo.variedad(s.variedadId)?.nombre ?? '—', String(s.lineas ?? '—'), numero(s.cantidad)])));
    ...
    if (app.length === 0) {
        ...
        raiz.append(tabla(['Fecha', 'Variedad', '# líneas', 'Cantidad'], app.map((l) => [l.fecha, ctx.catalogo.variedad(l.variedadId)?.nombre ?? '—', String(l.lineas ?? '—'), numero(l.cantidad)])));
    }
    return raiz;
}
