 ...
 */
import { resolverConflicto, resumenOutbox, sincronizar } from '../sincronizacion.js';
import { aviso, boton, cifra, etiqueta, h, tabla, titulo, vaciar, vacio } from '../vista.js';
import { numero } from './comun.js';
export async function pantallaBandeja(ctx) {
    const raiz = h('div', {});
    ...
    const pintar = async () => {
        ...
        cuerpo.append(h('div', { clase: 'cifras' }, cifra(r.pendientes, 'sin enviar'), cifra(r.enviados, 'enviados'), cifra(r.conflictos, 'en conflicto'), cifra(enServidor.length, 'camas en el servidor')));
        cuerpo.append(h('div', { clase: 'acciones' }, boton('Sincronizar', async () => {
            const res = await sincronizar(ctx.repo, { demoraMs: 120 });
            await pintar();
            ctx.refrescarShell();
            cuerpo.prepend(aviso(res.conflictos ? 'aviso' : 'ok', `${res.enviados} enviadas, ${res.conflictos} en conflicto.`, res.detalle[0] ?? ''));
        }), 
        ...
        boton('Sincronizar sin señal', async () => {
            ...
            await pintar();
            cuerpo.prepend(aviso('aviso', `${res.fallidos} quedaron en la bandeja.`, 'Nada se perdió: se reintenta cuando haya conexión.'));
        }, 'tenue')));
        if (conflictos.length > 0) {
            cuerpo.append(h('h2', { clase: 'seccion__titulo', texto: 'Decide una persona' }));
            for (const c of conflictos)
                cuerpo.append(await tarjetaConflicto(ctx, c, pintar));
        }
...
async function tarjetaConflicto(ctx, c, pintar) {
    ...
    const suma = (ls) => ls.reduce((t, l) => t + (l.cantidad ?? 0), 0);
    return h('div', { clase: 'seccion seccion--conflicto' }, h('div', { clase: 'seccion__cabeza' }, h('span', { clase: 'seccion__variedad', texto: `Cama ${cama?.codigo ?? '—'} · ${c.fecha}` }), etiqueta('alerta', 'conflicto')), h('p', { clase: 'nota', texto: 'Las dos capturas se conservan. Ninguna se sobreescribió sola.' }), tabla(['Versión', 'Secciones', 'Plantas'], [
    ...
    ]), h('div', { clase: 'acciones' }, boton('Queda la de este dispositivo', async () => {
        await resolverConflicto(ctx.repo, c, 'local');
        await pintar();
        ctx.refrescarShell();
    }), boton('Queda la que ya estaba', async () => {
        await resolverConflicto(ctx.repo, c, 'servidor');
        await pintar();
        ctx.refrescarShell();
    }, 'tenue')));
}
