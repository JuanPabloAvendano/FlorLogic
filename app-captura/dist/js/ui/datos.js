 ...
 * dónde lo sacaron?».
 */
import { aviso, boton, cifra, etiqueta, h, tabla, titulo } from '../vista.js';
export async function pantallaDatos(ctx, alBorrar) {
    ...
    raiz.append(h('div', { clase: 'cifras' }, cifra(r.bloques, 'bloques'), cifra(r.camas, 'camas'), cifra(r.variedades, 'variedades'), cifra(r.siembras, 'registros'), cifra(r.divididas, `camas divididas (${r.porcentajeDivididas}%)`), cifra(r.revisar, 'filas a revisar')));
    raiz.append(h('h2', { clase: 'seccion__titulo', texto: 'En este dispositivo' }));
    raiz.append(h('div', { clase: 'cifras' }, cifra(local.capturas, 'capturas'), cifra(local.borradores, 'borradores'), cifra(local.lineas, 'secciones'), cifra(local.pendientes, 'sin enviar')));
    raiz.append(h('div', { clase: 'acciones' }, boton('Borrar lo capturado en la demo', alBorrar, 'peligro')));
    ...
    for (const a of s.anomalias.filter((x) => x.severidad === 'revisar')) {
        raiz.append(h('p', { clase: 'aviso' }, etiqueta('aviso', a.tipo), ' ' + a.detalle, a.hoja ? h('span', { clase: 'aviso__origen', texto: ` — ${a.hoja}, fila ${a.fila}` }) : null));
    }
    ...
    for (const a of s.avisos)
        raiz.append(h('p', { clase: 'aviso aviso--neutro', texto: a }));
    raiz.append(h('h2', { clase: 'seccion__titulo', texto: 'Plantas por línea derivadas' }));
    raiz.append(tabla(['Variedad', 'Tipo', 'P/línea', 'Confianza'], s.variedades.map((v) => [
        v.nombre,
        v.tipoFlor || '—',
    ...
    raiz.append(aviso('neutro', `${ctx.motor.version()} · tolerancia de la razón: ${Math.round(ctx.motor.tolerancia() * 100)}%.`, 'Se cambian en configuracion/reglas.v1.json, sin tocar la aplicación.'));
    raiz.append(tabla(['Id', 'Severidad', 'Qué revisa'], ctx.motor.reglas().map((rg) => [
        rg.id,
