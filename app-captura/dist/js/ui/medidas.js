 ...
 */
import { csvDeEventos, csvDeMedidas, descargar, medidasPorCaptura, resumir } from '../metricas.js';
import { aviso, boton, cifra, etiqueta, h, tabla, titulo, vacio } from '../vista.js';
const META_SEGUNDOS = 30;
export async function pantallaMedidas(ctx) {
    ...
    raiz.append(h('div', { clase: 'cifras' }, cifra(r.camas, 'camas cerradas'), cifra(r.segundosMediana ?? '—', 'segundos (mediana)'), cifra(r.segundosPeor ?? '—', 'segundos (la peor)'), cifra(r.toquesMediana ?? '—', 'toques (mediana)'), cifra(r.rechazos, 'rechazos de regla')));
    if (r.segundosMediana !== null) {
        const cumple = r.segundosMediana <= META_SEGUNDOS;
        raiz.append(aviso(cumple ? 'ok' : 'aviso', cumple
            ? `La mediana está en ${r.segundosMediana} s, por debajo de los ${META_SEGUNDOS} s que pide RND.`
            : `La mediana está en ${r.segundosMediana} s, por encima de los ${META_SEGUNDOS} s que pide RND.`, 'Ese número es el que va en la medida de respuesta del escenario.'));
    }
    ...
    if (variantes.length > 1) {
        ...
        raiz.append(tabla(['Variante', 'Camas', 'Segundos (mediana)'], variantes.map(([nombre, v]) => [nombre, String(v.camas), String(v.segundosMediana ?? '—')])));
    }
    raiz.append(h('h2', { clase: 'seccion__titulo', texto: 'Cama por cama' }));
    raiz.append(tabla(['Cama', 'Variante', 'Segundos', 'Toques', 'Secciones', 'Rechazos'], medidas.map((m) => [
    ...
    raiz.append(h('div', { clase: 'acciones' }, boton('Descargar medidas (CSV)', () => descargar(`florlogic-medidas-${sello}.csv`, csvDeMedidas(medidas))), boton('Descargar eventos (CSV)', () => descargar(`florlogic-eventos-${sello}.csv`, csvDeEventos(eventos)), 'tenue')));
