...
export function medidasPorCaptura(eventos) {
    const porCaptura = new Map();
...
export function resumir(medidas) {
    const cerradas = medidas.filter((m) => m.cerrada && m.segundos !== null);
    const segundos = cerradas.map((m) => m.segundos);
    const porVariante = {};
    ...
    for (const clave of Object.keys(porVariante)) {
        porVariante[clave].segundosMediana = mediana(cerradas.filter((m) => (m.variante ?? 'sin variante') === clave).map((m) => m.segundos));
    }
    return {
...
export function csvDeEventos(eventos) {
...
export function csvDeMedidas(medidas) {
    const cabecera = ['capturaId', 'camaId', 'variante', 'segundos', 'toques', 'secciones', 'rechazos', 'cerrada'];
    const filas = medidas.map((m) => [m.capturaId, m.camaId, m.variante, m.segundos, m.toques, m.secciones, m.rechazos, m.cerrada ? 'sí' : 'no']
        .map(escapar)
        .join(';'));
    return [cabecera.join(';'), ...filas].join('\r\n');
}
